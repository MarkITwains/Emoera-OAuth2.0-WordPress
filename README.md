...
1. 下载本仓库，把 `emoera-openid` 文件夹放到站点的 `wp-content/plugins/` 下；
   也可以把整个仓库打成一个 zip，在 **插件 → 安装插件 → 上传插件** 中直接安装。
2. 在 **插件** 列表里启用「E时代通行证」。
3. 左侧菜单出现 **E时代通行证**，进入并填写配置（见下一节）。
4. 把页面里显示的 **回调地址** 提交给通行证服务方登记。
5. 退出登录，在登录页点「使用E时代通行证登录」实测一次。
